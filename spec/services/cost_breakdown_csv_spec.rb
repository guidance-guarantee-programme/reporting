...
RSpec.describe CostBreakdownCsv do
  let(:separator) { ',' }
  let(:month_with_data) { double('data', month: '2016-06', count: 1, value: 100, forecast: true) }
  let(:month_without_data) { double('data', month: '2016-07', count: 0) }
  let(:record) do
    double(
      'Costs::Item',
      name: 'Twilio',
